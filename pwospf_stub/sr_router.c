...
/*
 * Fetch the OSPF_HDR from the Packet
 */
struct ospfv2_hdr *get_ospf_hdr(uint8_t *packet)
{
	return  ((struct ospfv2_hdr*)(packet + sizeof(struct sr_ethernet_hdr) + sizeof(struct ip)));
}
/*
...
void send_cached_packet(uint32_t ip_addr)
{
	...
	printf("\n ARP Hdr Src Ip :: ");printIpAddress(ip_addr);
	...
	while(current_packet != NULL)
	{
		...
		if( (ip_hdr->ip_ttl > 1 && ip_addr == dst_ip) || (ip_hdr->ip_ttl > 1 && is_ip_for_gate_way && current_packet->is_packet_for_gateway))
		{
			...
			{
				fetchedInterface = getInterFaceForIpDynamic(packet_found->sr,dst_ip);
				sr_route_packet_Dynamic_mode(packet_found->sr,packet_found->packet,packet_found->length,packet_found->interface,fetchedInterface);
			}
...
struct sr_rt* getInterFaceForIp(struct sr_instance* sr,uint32_t ipAddToMatch)
{
	...
	printf("\nFetch Iface for IP :: %s",inet_ntoa(*(struct in_addr*)(&ipAddToMatch)));
	struct sr_rt* rtable;
	...
	while(rtable)
	{
		...
		// PWOSPF GATEWAY WILL NOT BE 0.0.0.0 this condition is wrong --  rtable->gw.s_addr ==0 &&

		if(rtable->mask.s_addr > 0 && ((rtable->mask.s_addr & ipAddToMatch) == (rtable->dest.s_addr & rtable->mask.s_addr)))
		{
...
struct sr_rt* getInterFaceForIpDynamic(struct sr_instance* sr,uint32_t ipAddToMatch)
{
	printf("\nInside getInterFaceForIpDynamic()\n");
	printf("\nFetch Iface for IP :: %s",inet_ntoa(*(struct in_addr*)(&ipAddToMatch)));
	struct sr_rt* rtable;
	//pthread_mutex_lock(&(sr->ospf_subsys->lock));
	rtable = sr->routing_table;
		...
		char* interface/* lent */)
{
	...
	{
		//printf("\n Dynamic Mode\n");
		sr_handlepacket_dynamic(sr,packet,len,interface);
	}
	else
	{
		printf("\n Static Mode\n");
		sr_handlepacket_static(sr,packet,len,interface);
	}

}
void sr_route_packet_Dynamic_mode(struct sr_instance * sr, uint8_t * packet, int len, char* interface,struct sr_rt* fetchedInterface)
{
	printf("\nDynamice Routing the packet ");
	...
	uint32_t dst_ip;
	printf("\nDestination Ip ::");printIpAddress(ip_header->ip_dst.s_addr);
	if(fetchedInterface != NULL && fetchedInterface->gw.s_addr != 0)
	{
		dst_ip = fetchedInterface->gw.s_addr;
		printf("\n ****DST_IP ::");printIpAddress(dst_ip);
		if(dst_ip == 0 || dst_ip <=0)
		{
			dst_ip = ip_header->ip_dst.s_addr;
		}
		printf("\n Fetched Iface was NOT NULL, DST_IP ::");printIpAddress(dst_ip);// MOSTLY THE CASE WHERE SERVER IS REPLYING TO GATEWAY sAddr will be server 172.29.5.39

	}
	else
	{
		dst_ip = ip_header->ip_dst.s_addr;
		printf("\n Fetched Iface was NULL, DST_IP ::");printIpAddress(dst_ip);

	}
	//should use the dest prefix for PWOSPF ip_header->ip_dst.s_addr;//This is destination Address
	...
	if(mac_addr_cached[0] == '\0')// Cache Miss
	{
		...
		struct sr_rt* doesInterfaceExistForSourceIp = getInterFaceForIpDynamic(sr,src_ip);// If Source is on the Network (Skip gateway here)
...
void printIpAddress(uint32_t ipAddr)
{
	printf(" %s ",inet_ntoa(*(struct in_addr*)(&ipAddr)));
}

uint16_t calculateCheckSumHelloPacket(uint8_t* hdr, int len)
{
    long sum = 0;
...
void handlePWOSPFPacket(struct sr_instance* sr,uint8_t * packet/* lent */,unsigned int len,char* interface/* lent */)
{

	struct ospfv2_hdr* ospfHdr = ((struct ospfv2_hdr*)(packet + sizeof(struct sr_ethernet_hdr) + sizeof(struct ip)));
	//printf("\n HandlePwOSPFPacket ospf Hdr-Type :: %d",ospfHdr->type);
	switch(ospfHdr->type)
	{
		case HELLO_PACKET:
			handleHelloPackets(sr,packet,len,interface);
			break;
		case LSU_PACKET:
			handleLsuPacket(sr,packet,len,interface);
			break;
	}
...
/*
 * This code will handle packets in dynamic mode PWOSPF
 */
void sr_handlepacket_dynamic(struct sr_instance* sr,
		uint8_t * packet/* lent */,
		unsigned int len,
		char* interface/* lent */)
{
	...
	case ETHERTYPE_IP:
		...
		if(ip_hdr != NULL)
		{
			if(ip_hdr->ip_p == IP_PROTO_OSPFv2)
			{
				handlePWOSPFPacket(sr,packet,len,interface);
				return;
			}
		}
		struct sr_icmphdr* received_icmp = (struct sr_icmphdr*)(packet+sizeof(struct sr_ethernet_hdr)+sizeof(struct ip));
		...
		printf("\n--Received IP Destin addr :: %s",inet_ntoa(*(struct in_addr*)(&dst_ip)));
		//printf("\n A");
		...
		{
			fetchedInterface = getInterFaceForIpDynamic(sr,dst_ip);
			printf("\n Done with IFACE");
			sr_route_packet_Dynamic_mode(sr,packet,len,interface,fetchedInterface);
		}
...
/*
 * This code will handle packets in static mode [ OLD CODE ]
 */
void sr_handlepacket_static(struct sr_instance* sr,
		uint8_t * packet/* lent */,
		unsigned int len,
		char* interface/* lent */)
{
	...
	case ETHERTYPE_IP:
		...
		if(ip_hdr != NULL)
		{
			if(ip_hdr->ip_p == IP_PROTO_OSPFv2)
			{
				struct sr_if* ifacePwospf = sr_get_interface(sr, interface);
				handlePWOSPFPacket(sr,packet,len,interface);
				return;
			}
		}
...
/*
 * Cache MAC ADDRESS for a given IP and MAC
 */
...
{
	int i;
	int isFromCacheMac = 2;// 0 MEANS NO , 1 MEANS YES , 2 MEANS it is PWOSPF
	time_t timeNow;
	struct timeval tv;
	//printf("\nInside cache_mac_address\n");
	struct ip_mac_cache cache ;
	unsigned char*  existing_mac_addr ;
	existing_mac_addr = does_mac_address_exist(ipHdr->ip_src.s_addr,isFromCacheMac);
	if(existing_mac_addr[0] != '\0')
	{
		//printf("\n [Dynamic]MAC Exists Not Performing Cache");
		for(i=0; i<6; i++)
		{
		//	printf("%x:", existing_mac_addr[i]);
		}
	...
	//printf("\n [Dynamic]Caching MAC for ip_address :: %s",inet_ntoa(*(struct in_addr*)(&ipHdr->ip_src.s_addr))); //Source is SERVER
	cache.ip_address = ipHdr->ip_src.s_addr;
	memcpy(cache.mac_address, ethHdr->ether_shost,ETHER_ADDR_LEN);
	gettimeofday(&tv, NULL);
	cache.entryTime = tv.tv_sec + (tv.tv_usec/1000000.0);
	cache.valid =1;
	//printf("\n savinf to cache time entry  :: %f",cache.entryTime);
...
void send_arp_request_PWOSPF(struct sr_instance * sr, uint32_t dst_ip, char* interface,struct sr_rt* fetchedInterface)
{
	printf("\nSending ARP Request PWOSPF \n");
